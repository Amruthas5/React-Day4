...
const App = () => {
    ...
    const changeItem = (index)=>{
        if(winMessage){
            return toast("Game over,try again", {type: "success"})
        }
    ...
    backgroundColor:"red"}} onClick={playAgain}> Play Again</Button>
                        </div>
                    ) : (
                        <h2 style={{padding:"5px",textAlign:"center",
                       ...
                       {tiktacArray.map((value,index)=>(
                           <Card onClick={()=>changeItem(index)}> 
                               <CardBody className="box"> 
                                   <Symbol choice={tiktacArray[index]}/>
                               </CardBody>
                           </Card>
                       ))}
